...
public class MotorTestComp extends CommandBase {
  ...
  private double[] speedDiff = new double[200];
  private double[] distDiff = new double[200];
  private double aveSpeedDiff;
  private double aveDistDiff;
  ...
  public MotorTestComp(Drivetrain drive) {
  ...
  public void initialize() {
    ...
    SmartDashboard.putNumber("CompSpeed7", m_drive.compSpeed[7]);
    n = 0;
    ...
    String name = Integer.toString((int) m_startTime) + ".txt";
  ...
  public void execute() {
    ...
    if (Timer.getFPGATimestamp() - m_startTime > stepTime) {
      ...
      SmartDashboard.putNumber("StepN", n);
    }

    speedDiff[n] += (m_drive.getLeftSpeed() - m_drive.getRightSpeed());// sum difference in speeds


    SmartDashboard.putNumber("SDiff", speedDiff[n]);
    if (stepChange) {
      SmartDashboard.putBoolean("STEPCH", stepChange);
  
      m_drive.compSpeed[n] = speedDiff[n];
      stepChange = false;
    }
